...
for c in Ωc:
    for a in Ωa:
        plt.figure()
        plt.plot(Ωt, [PS_values[t, c, a] for t in Ωt], label="PS")
        plt.plot(Ωt, [PGD_values[t, c, a] for t in Ωt], label="PGD")
        plt.plot(Ωt, [PD[t] for t in Ωt], label="PD")
        plt.plot(Ωt, [fPV[t] * PPVmax_value for t in Ωt], label="fPV")
        plt.plot(Ωt, [PAEc_values[t, c, a] for t in Ωt], label="PAEc")
        plt.plot(Ωt, [-1*PAEd_values[t, c, a] for t in Ωt], label="PAEd")
        plt.plot(Ωt, [PEVc_values[t, c, a] for t in Ωt], label="PEVc")
        plt.plot(Ωt, [-1*PEVd_values[t, c, a] for t in Ωt], label="PEVd")
        plt.legend()
        plt.xlabel("Timestamp")
        plt.ylabel("Power [kW]")
        plt.savefig(f"Results/operation_c{c}_a{a}.png")
        plt.close()
