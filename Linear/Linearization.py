...
# Set MIPGap parameter to 0.001 (0.1%)
...
PSp   = {(t, s, c, a): model.addVar(name=f"PSp_{t}_{c}_{a}", lb=0, ub=par['baseModel']['PSmax'])             for t in Ωt for s in Ωs for c in Ωc for a in Ωa} # Substation Power in time t, contingency c, and EV scenario a
PSn   = {(t, s, c, a): model.addVar(name=f"PSn_{t}_{c}_{a}", lb=0, ub=par['baseModel']['PSmax'])             for t in Ωt for s in Ωs for c in Ωc for a in Ωa} # Substation Power in time t, contingency c, and EV scenario a
...
for t in Ωt:
    for c in Ωc:
        for a in Ωa:
            if (t < c):
                model.addConstr(SoCEV[t, c, a] == SoCEV[t, 0, a])
                model.addConstr(γEVc[t, c, a] == γEVc[t, 0, a])
                model.addConstr(γEVd[t, c, a] == γEVd[t, 0, a])
                model.addConstr(PGD[t, c, a] == PGD[t, 0, a])
                model.addConstr(EAE[t, c, a] == EAE[t, 0, a])
                model.addConstr(γAEc[t, c, a] == γAEc[t, 0, a])
                model.addConstr(γAEd[t, c, a] == γAEd[t, 0, a])
...
for t in Ωt:
    for s in Ωs:
        for c in Ωc:
            for a in Ωa:
                if (t < c):
                    model.addConstr(PS[t, s, c, a] == PS[t, s, 0, a])
                    model.addConstr(PSp[t, s, c, a] == PSp[t, s, 0, a])
    ...
    for c in Ωc:
        for a in Ωa:
            if t < Ωa[a]['arrival'][0]:
                ...
                model.addConstr(PEVc[t,c,a] <= par['baseModel']['EVPmaxc'] * γEVc[t,c,a], name=f"EV_ChargeMax_{t}_{c}_{a}")
                model.addConstr(PEVd[t,c,a] <= par['baseModel']['EVPmaxd'] * γEVd[t,c,a], name=f"EV_DischargeMax_{t}_{c}_{a}")
                model.addConstr(γEVc[t,c,a] + γEVd[t,c,a] <= 1, name=f"EV_Charge_Discharge_condition_{t}_{c}_{a}")

                # Linearize SoCEV calculation
                for n in range(len(Ωa[a]['arrival'])):
                    if t == Ωa[a]['departure'][n]:
                        model.addConstr(SoCEV[t, c, a] == 1, name=f"EV_SoC_end_{t}_{c}_{a}")
                    elif t >= Ωa[a]['arrival'][n] and t < Ωa[a]['departure'][n]:
                        model.addConstr(SoCEV[t, c, a] >= Ωa[a]['SoCini'][n] + Δt * (PEVc[t,c,a] - PEVd[t,c,a])/Ωa[a]['Emax'][n], name=f"EV_SoC_{t}_{c}_{a}")
                    elif t == Ωa[a]['arrival'][n]:
                        model.addConstr(SoCEV[t, c, a] == Ωa[a]['SoCini'][n], name=f"EV_SoC_ini_{t}_{c}_{a}")
                    elif t == Ωa[a]['departure'][n]:
                        model.addConstr(SoCEV[t, c, a] == 1, name=f"EV_SoC_end_{t}_{c}_{a}")
                    elif t > Ωa[a]['departure'][n] and t < Ωa[a]['arrival'][n+1]:
...
for t in Ωt:
    for c in Ωc:
        for a in Ωa:
            if t > 1:
                model.addConstr(
                    EAE[t, c, a] == EAE[t - 1, c, a] + par['baseModel']['alpha'] * Δt * PAEc[t, c, a] - Δt * PAEd[t, c, a] / par['baseModel']['alpha'] - par['baseModel']['beta'] * Δt * EAE[t, c, a],
                    name=f"Energy_Storage_Balance_{t}_{c}_{a}"
            )
            if t == 1:
                model.addConstr(
                    EAE[t, c, a] == par['baseModel']['EAE0'] * EAEmax + par['baseModel']['alpha'] * Δt * PAEc[t, c, a] - Δt * PAEd[t, c, a] / par['baseModel']['alpha'] - par['baseModel']['beta'] * Δt * EAE[t, c, a],
...
for s in Ωs:
    for c in Ωc:
        for a in Ωa:
            ...
            plt.plot(Ωt, [fp[s]['load'][t-1]*par['baseModel']['MaxL'] for t in Ωt], label="Demand")
            ...
            plt.ylabel("Power [kW]")
            plt.savefig(f"Results_Teste/operation_s{s}_c{c}_a{a}.png")
            plt.close()
...
for c in Ωc:
    for a in Ωa:
        ...
        plt.xlabel("Timestamp")
        plt.ylabel("SoC")
        plt.savefig(f"Results_Teste/storage_c{c}_a{a}.png")
        plt.close()
